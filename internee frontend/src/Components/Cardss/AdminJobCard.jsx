...
import { usestore } from "../../Store/ContextStore";
 const AdminJobCard = ({ image, title, location, duration, type, remote, id, job,setjob}) => {
   ...
   const handledelete=(details)=>{
    ...
    return (
        <div className=" flex  items-center justify-between bg-gray-100 shadow-[35px] shadow-gray-900 rounded-xl py-10 px-7 w-full my-5">
        {/* Left Side: Logo & Info */}
        <div className="flex items-center gap-4">
          {/* Job Image */}
          <img src={image} alt={title} className="w-20 h-20 rounded-full object-fill" />
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={()=>{
                openPopup({id:id,Title:title})
            }}
            className="bg-red-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-green-600 transition"
          >
            Delete
          </button>
        </div>
